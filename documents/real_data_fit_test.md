...
- [ ] **相互作用項の定式化そのものの見直し(最優先)**: 駆動力側の調整では解決しない構造的な問題であることが確定した。指数関数的な力の形自体、役割クラスの分け方、「攻撃者1人+守備者集団」という単純化された系の設計を再検討する必要がある。ここでのユーザー提案(横山論文の空間力(spatial force)、ピッチコントロールに基づくポテンシャル場$e_i(t)=\nabla\Phi_i/\|\nabla\Phi_i\|$)も、駆動力の再設計としてだけでなく、相互作用項側の再設計のヒントとして再検討する価値がある。
- [ ] **合成データでの再検証**: `synthetic_recovery_test.md`の合成データテストでは$A,B$の復元に成功していた。今回の実データでの説明力不足が「データが複雑すぎる」ためか「駆動力とのせめぎ合いが実データ特有の問題」なのかを切り分けるため、合成データに同様の(τを自由にした)プール推定を適用し、実データと同じ縮退が起きるかを確認する価値がある。
