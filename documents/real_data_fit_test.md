...
- [x] ~~$v_0 e_i(t)$の外部情報ベースへの再設計~~ → 実施済み。リークは排除できたが、駆動力の近似(一定速度直進)が単純化しすぎており、フィット精度・収束性ともに悪化。**再設計はこれで終わりではなく、続きが必要**と判明。
- [ ] **駆動力設計の中間案の検討(最優先)**: 完全自己参照(過学習しやすい)と完全固定(単純化しすぎ)の間を探る。例: $v_0$自体を推定対象にする、ゴール方向に加えてボール位置による補正を加える、より短い時間窓で「直進」の近似が成り立つ範囲に絞る、など。
- [ ] **局面数を増やす(12→30〜50程度)**: どの駆動力設計を採用するにせよ、局面の多様性を増やすこと自体は引き続き有効と考えられる。
- [ ] $A_1, B_1$の識別性向上、$A_2$の符号解釈の照合(7.2節)は、駆動力設計が定まった後に再度取り組む。
