...
- [ ] **EMA半減期の感度分析(最優先)**: 0.3秒より長い半減期(0.5秒, 1.0秒など)で同じプール推定を行い、$\tau$が制約の内側で自然に収束するか、$A,B$の値がどれだけ半減期に敏感かを確認する。
- [ ] **局面数を増やす(12→30〜50程度)**: 半減期の妥当な設定が定まった後、局面の多様性を増やして$A,B$の安定性をさらに確認する。
...
- `scripts/real_data_pooled_fit_v3.py` — 12局面プール推定(因果的EMA版)
- `scripts/real_data_pooled_fit_v3_result.json` — 実行結果(因果的EMA版)
