...
- [ ] **k分割交差検証、または局面数自体を増やしてtestサンプルを大きくする(優先)**: 追試15でtest改善率の推定誤差が大きい(std=4.1%)ことが判明した。10局面固定のtrain/test分割ではなく、全局面をtestに回すk分割交差検証にするか、複数試合プールで絶対的な局面数を増やし、改善率をより precision良く報告できるようにする。
