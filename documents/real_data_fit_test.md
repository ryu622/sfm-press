...
- [x] ~~駆動力信号を実際の意図により近づける(動的ポテンシャル・簡易版)~~ → 実施済み。$\tau$固定時の改善率が因果的EMA版の最大7.3%から32.8%まで向上(同じτ=1.2秒で比較)。相互作用項が説明できる割合という観点では明確な前進。ただし絶対的なフィット精度はEMA版より悪化し、パラメータもτ設定間で不安定($A_1$の符号反転など)。
- [ ] **局面数を増やしてパラメータの安定性を確認する(最優先)**: 動的ポテンシャル版はτに対して改善率が山型のピークを見せ(際限のない縮退ではない)良い兆候だが、$A_1$の符号反転などパラメータがτ設定間で不安定。局面数を12→30〜50程度に増やし、この不安定性が局面数不足によるものか、設計自体の問題かを確認する。
- [ ] **守備者の駆動力モデルの精緻化**: 「攻撃者の現在位置を追いかける」という単純な追跡は、絶対的なフィット精度をEMA版より悪化させた。先回り・カバーリングなど、より現実的な守備の動きを表現する駆動力設計を検討する(ユーザー提案のポテンシャル場$\Phi$による、より豊かな空間評価指標も選択肢)。
...
- `scripts/tau_fixed_v4_sweep.py` — τ固定・動的ポテンシャル版の説明力テスト(4段階)
- `scripts/tau_fixed_v4_sweep_result.json` — 実行結果(τ固定・動的ポテンシャル版)
- `scripts/plot_tau_fixed_v4_sweep.py` — 図の生成スクリプト(EMA版との比較)
- `documents/figures/tau_fixed_v4_vs_ema.png` — EMA版と動的ポテンシャル版の比較画像
