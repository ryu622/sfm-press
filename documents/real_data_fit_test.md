...
- [ ] **駆動力信号の独立性をさらに高める(最優先)**: 因果的EMAも「答えの延長線上をなぞる」余地を残していたと考えられる。EMAをやめて完全に外部情報のみ(追試3のゴール方向)に、ただし追試3の「一定速度」という単純化しすぎた点だけを改善する(例: $v_0$を推定対象にする、味方選手の平均位置や過去の局面の平均速度など集団統計量から$v_0$を与える)方向を検討する。
- [ ] **駆動力自体を極端に弱くする・切る実験**: 逆に、駆動力を意図的にほぼ無力化する($\tau$を非常に大きい値に固定する、または駆動力項自体を0にする)極端な設定で、相互作用項$A,B$だけでどこまで軌道を説明できるかを確認する。SFMの相互作用項に「守備プレッシャー」を説明する力が本当にあるかどうかへの、より直接的な検証になる。
- [ ] $A_2$の符号(負=引力的、仮説と整合)は、$\tau$の縮退問題が解決するまで暫定的な結果として扱う。7.2節の検証(既存指標との照合)は駆動力設計の確定後に行う。
