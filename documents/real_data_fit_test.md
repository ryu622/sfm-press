...
- [ ] **$\tau$の下限制約自体を緩めた実験(最優先)**: $\text{TAU\_MIN}=0.3$秒をさらに下げて(例: 0.05秒)、$\tau$がどこで自然に落ち着くかを確認する。歩行者分野の値に固執せず、データが示す値をまず観察する。
- [ ] **局面数を増やす(12→30〜50程度)**: 半減期0.3秒(最もフィットが良かった設定)のまま、局面の多様性を増やして$A,B,\tau$の安定性を確認する。
- [ ] $A_2$の符号(負=引力的、仮説と整合)について、既存のプレッシャー指標・ケーススタディとの照合(7.2節の検証計画)で確認する。
